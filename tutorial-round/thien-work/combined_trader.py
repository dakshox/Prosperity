...
class Trader:
    ...
    def starfruit_strategy(state, data):
        ...
        buy_target = min(data.ema0_1, target_price) - 1.5
        sell_target = max(data.ema0_1, target_price) + 1.5
        # print(ema_diff, pred_future_diff, target_price)

        # correction to hold polarised positions when market is moving
        # if pred_future_diff > 0.8 and position < 13:
        #     buy_target += 0.5
        #     sell_target += 0.5
        # if pred_future_diff < -0.8 and position > -13:
        #     buy_target -= 0.5
        #     sell_target -= 0.5
        # if position >= 17 and pred_future_diff < -0.4:
        #     sell_target += (data.ema0_1 - sell_target) * 0.35
        # if position <= -17 and pred_future_diff > 0.4:
        #     buy_target += (data.ema0_1 - buy_target) * 0.35

        orders = []

        if position > 16 and pred_future_diff < -0.1:
            sell_price = sell_target + (data.ema0_1 - sell_target) * 0.4
            qty = min(max_sell, position - 16)
            orders.append(Order(symbol, round(sell_price), -qty))
            max_sell -= qty
        if position < -16 and pred_future_diff > 0.1:
            buy_price = buy_target + (data.ema0_1 - buy_target) * 0.4
            qty = min(max_buy, 16 - position)
            orders.append(Order(symbol, round(buy_price), qty))
            max_buy -= qty
        ...
        # some market making
        if max_buy > 0:
            orders.append(Order(symbol, round(buy_target), max_buy))
        if max_sell > 0:
            orders.append(Order(symbol, round(sell_target), -max_sell))
